...
@section('content')
    ...
    {!! Form::model($commandes_avec_compte, ['method' => 'PUT', 'route' => ['admin.commandes_avec_comptes.update', $commandes_avec_compte->id]]) !!}
        ...
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    ...
                    {!! Form::hidden('map_latitude', $commandes_avec_compte->map_latitude , ['id' => 'map-latitude']) !!}
                    {!! Form::hidden('map_longitude', $commandes_avec_compte->map_longitude , ['id' => 'map-longitude']) !!}
                    <p class="help-block"></p>
                    @if($errors->has('map'))
                        <p class="help-block">
                            {{ $errors->first('map') }}
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody id="inventaires-avec-compte">
                    @forelse(old('inventaires_avec_comptes', []) as $index => $data)
                        @include('admin.commandes_avec_comptes.inventaires_avec_comptes_row', [
                            'index' => $index
                        ])
                    @empty
                        @foreach($commandes_avec_compte->inventaires_avec_comptes as $item)
                            @include('admin.commandes_avec_comptes.inventaires_avec_comptes_row', [
                                'index' => 'id-' . $item->id,
                                'field' => $item
                            ])
