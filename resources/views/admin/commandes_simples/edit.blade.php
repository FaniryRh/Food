...
@section('content')
    ...
    {!! Form::model($commandes_simple, ['method' => 'PUT', 'route' => ['admin.commandes_simples.update', $commandes_simple->id]]) !!}
        ...
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    ...
                    {!! Form::hidden('map_latitude', $commandes_simple->map_latitude , ['id' => 'map-latitude']) !!}
                    {!! Form::hidden('map_longitude', $commandes_simple->map_longitude , ['id' => 'map-longitude']) !!}
                    <p class="help-block"></p>
                    @if($errors->has('map'))
                        <p class="help-block">
                            {{ $errors->first('map') }}
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody id="inventaires-simple">
                    @forelse(old('inventaires_simples', []) as $index => $data)
                        @include('admin.commandes_simples.inventaires_simples_row', [
                            'index' => $index
                        ])
                    @empty
                        @foreach($commandes_simple->inventaires_simples as $item)
                            @include('admin.commandes_simples.inventaires_simples_row', [
                                'index' => 'id-' . $item->id,
                                'field' => $item
                            ])
